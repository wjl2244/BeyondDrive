...
experiment_name=diffusiondrive_beyonddrive  # replace with your experiment name
experiment_uid=$(date +%Y.%m.%d.%H.%M.%S)
output_dir=exp/diffusiondrive_training/${experiment_name}/${experiment_uid}

lr=2e-4
batch_size=8  # 8 for training on 8 GPU
