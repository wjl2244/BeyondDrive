...
from navsim.agents.transfuser.utils import diff_traj
...
def transfuser_loss(
    ...
    gt_trajectory = targets["trajectory"].to(dtype)
    
    rde_loss = 0.0

    if config.use_delta_traj:
        trajectory_loss = F.l1_loss(predictions['delta_trajectory'], diff_traj(gt_trajectory))
        if config.use_beyonddrive:
            if 'pdm_score' not in predictions:
                negative_index = (targets['negative_trajectory'].sum(-1).sum(-1) > 0)
                delta_negative_trajectory = diff_traj(targets['negative_trajectory'])
                rde_loss = ((predictions['delta_trajectory']-delta_negative_trajectory).abs().mean(-1).mean(-1))[negative_index].mean()
    else:
        trajectory_loss = F.l1_loss(predictions['trajectory'], gt_trajectory)
        if config.use_beyonddrive:
            trajectory_loss = F.l1_loss(predictions['trajectory'], gt_trajectory)
            if 'pdm_score' not in predictions:
                negative_index = (targets['negative_trajectory'].sum(-1).sum(-1) > 0)
                rde_loss = ((predictions['trajectory']-targets['negative_trajectory']).abs().mean(-1).mean(-1))[negative_index].mean()
    rde_loss = -rde_loss        
    
    if 'agent_states' in predictions:
        agent_class_loss, agent_box_loss = _agent_loss(targets, predictions, config)
    else:
        agent_class_loss, agent_box_loss = 0.0, 0.0
    ...
    loss = (
        ...
        + config.rde_loss_weight * rde_loss
    )
    loss_dict.update({
        ...
        "rde_loss": config.rde_loss_weight * rde_loss,
        "loss": loss,
        })
