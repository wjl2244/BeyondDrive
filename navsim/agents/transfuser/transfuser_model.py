...
from navsim.agents.transfuser.utils import cumsum_traj
...
class TransfuserModel(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        self._trajectory_head = TrajectoryHead(config)
...
class TrajectoryHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self._d_ffn = config.tf_d_ffn
        self._use_delta_traj = config.use_delta_traj

        self.traj_dim = StateSE2Index.size()
        if self._use_delta_traj:
            self.traj_dim += 1
        self._mlp = nn.Sequential(
            nn.Linear(self._d_model, self._d_model),
            nn.SiLU(),
            nn.Linear(self._d_model, self._num_poses * self.traj_dim),
        )

    def forward(self, object_queries) -> Dict[str, torch.Tensor]:
        """Torch module forward pass."""
        poses = self._mlp(object_queries).reshape(-1, self._num_poses, self.traj_dim)
        
        if self._use_delta_traj:
            delta_trajectory = poses
            trajectory = cumsum_traj(delta_trajectory)
            return {"trajectory":trajectory, "delta_trajectory": delta_trajectory}
        else:
