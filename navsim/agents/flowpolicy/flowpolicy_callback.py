...
from navsim.agents.flowpolicy.utils import BoundingBox2DIndex
...
class TransfuserCallback(pl.Callback):
    ...
    def _visualize_model(
        ...
        pred_trajectory = predictions["pred_trajectorys"].numpy()
...
def dict_to_device(dict: Dict[str, torch.Tensor], device: Union[torch.device, str]) -> Dict[str, torch.Tensor]:
    ...
    for key in dict.keys():
        if isinstance(dict[key], torch.Tensor):
            ...
            for i, tensor in enumerate(dict[key]):
                if isinstance(tensor, torch.Tensor):
                    dict[key][i] = tensor.to(device)
    return dict
...
def lidar_map_to_rgb(
    ...
    # for color, traj in zip([gt_color, pred_color], [trajectory, pred_trajectory[0]]):
    ...
    trajectory_indices = coords_to_pixel(trajectory[:, :2])
    for x, y in trajectory_indices:
        cv2.circle(rgb_map, (y, x), point_size, gt_color, -1)  # -1 fills the circle

    for traj in pred_trajectory:
        trajectory_indices = coords_to_pixel(traj[:, :2])
        for x, y in trajectory_indices:
            cv2.circle(rgb_map, (y, x), 1, pred_color, -1)  # -1 fills the circle
