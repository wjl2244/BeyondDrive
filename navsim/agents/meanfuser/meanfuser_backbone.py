...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, config):
        ...
        try:
            ...
            if config.image_architecture.lower() == "resnet34":
                resnet_ckpt_path = config.resnet34_ckpt_path
            elif config.image_architecture.lower() == "resnet18":
                resnet_ckpt_path = config.resnet18_ckpt_path
            elif config.image_architecture.lower() == "resnet50":
                resnet_ckpt_path = config.resnet50_ckpt_path
            else:
                raise ValueError("Unsupported image architecture")
            self.image_encoder = timm.create_model(config.image_architecture, pretrained=True, features_only=True,
                                                   pretrained_cfg_overlay=dict(file=resnet_ckpt_path))
