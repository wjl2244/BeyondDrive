...
class MFFeatureBuilder(AbstractFeatureBuilder):
...
class MFTargetBuilder(AbstractTargetBuilder):
    def __init__(self, config: MeanfuserConfig):
        self._config = config
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        trajectory = torch.tensor(future_traj.poses, dtype=torch.float32)
    ...
    def _compute_agent_targets(self, annotations: Annotations) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        def _xy_in_lidar(x: float, y: float, config: MeanfuserConfig) -> bool:
