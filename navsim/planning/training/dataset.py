...
class CacheOnlyDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.submetric_index = int(os.environ.get("SUBMETRIC_INDEX", "-1"))
    ...
    def _load_scene_with_token(self, token: str) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        if self.is_training and self.use_beyonddrive:
            with open(f"{self.negative_samples_path}/{token}.pkl", "rb") as f:
                token_trajs_pdms = pickle.load(f)
            human_trajectory = targets['trajectory']
            # valid_index = (token_trajs_pdms['pdm_score_matrix'][:,-1] < 0.6)
            valid_index = (token_trajs_pdms['pdm_score_matrix'][:,self.submetric_index] < 0.6)  # DAC
            pred_trajectorys = token_trajs_pdms["pred_trajectorys"][valid_index]
            if pred_trajectorys.shape[0] > 0:
                pred_trajectorys = torch.from_numpy(pred_trajectorys).to(torch.float32)
                min_index = torch.argmin((pred_trajectorys-human_trajectory[None,]).pow(2).sum(dim=-1).mean(dim=-1))
                negative_trajectory = pred_trajectorys[min_index]
                error = (pred_trajectorys-human_trajectory[None,]).pow(2).sum(dim=-1).mean(dim=-1).min().item()
            else:
                negative_trajectory = human_trajectory * 0
