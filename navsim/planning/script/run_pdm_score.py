...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[Dict[str, Any]]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            # gt_trajectory = scene_loader.get_scene_from_token(token).get_future_trajectory().poses[:-2]
            # poese_reg = trajectory['poses_reg'][0]
            # best_index = torch.norm(poese_reg[...,:2]-gt_trajectory[...,:2], dim=-1).mean(dim=-1).argmin()
            # trajectory = Trajectory(poese_reg[best_index])
            trajectory = trajectory["trajectory"][0].numpy()
            # trajectory[...,-1] = gt_trajectory[...,-1]
