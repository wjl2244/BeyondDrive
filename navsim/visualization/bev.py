...
def add_configured_bev_on_ax(ax: plt.Axes, map_api: AbstractMap, frame: Frame, add_ego=True) -> plt.Axes:
    """
    ...
    if "annotations" in BEV_PLOT_CONFIG["layers"]:
        add_annotations_to_bev_ax(ax, frame.annotations, add_ego)
...
def add_trajectory_to_bev_ax(ax: plt.Axes, trajectory: Trajectory, config: Dict[str, Any], label=None) -> plt.Axes:
    """
